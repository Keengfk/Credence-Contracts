...
impl CredenceBond {
    ...
    pub fn set_tier_thresholds(
        ...
        if bronze_max <= 0 {
            panic!("bronze max must be positive");
        }
        if silver_max <= bronze_max {
            panic!("silver max must be greater than bronze max");
        }
        if gold_max <= silver_max {
            panic!("gold max must be greater than silver max");
        }

        let old_thresholds = e
            .storage()
            .instance()
            .get::<_, TierThresholds>(&DataKey::TierThresholds)
            .unwrap_or_else(|| TierThresholds {
                bronze_max: tiered_bond::TIER_BRONZE_MAX,
                silver_max: tiered_bond::TIER_SILVER_MAX,
                gold_max: tiered_bond::TIER_GOLD_MAX,
            });
        ...
        e.storage()
            .instance()
            .set(&DataKey::TierThresholds, &new_thresholds);

        e.events().publish(
            (Symbol::new(&e, "tier_thresholds_changed"),),
            (old_thresholds, new_thresholds),
        );
    }

    pub fn get_tier_thresholds(e: Env) -> TierThresholds {
        e.storage()
            .instance()
            .get::<_, TierThresholds>(&DataKey::TierThresholds)
            .unwrap_or_else(|| TierThresholds {
    ...
    pub fn withdraw_early(e: Env, amount: i128) -> IdentityBond {
        ...
        // Add a potential penalty refund claim (50% of penalty can be refunded for good behavior) (Effect)
        if penalty > 0 {
    ...
    pub fn top_up(e: Env, amount: i128) -> IdentityBond {
        ...
        Self::with_reentrancy_guard(&e, || {
            ...
            let normalized_amount = normalization::normalize(&e, &token_addr, amount);
            ...
            let old_tier = tiered_bond::get_tier_for_amount(&e, old_amount);
            let new_tier = tiered_bond::get_tier_for_amount(&e, new_amount);
